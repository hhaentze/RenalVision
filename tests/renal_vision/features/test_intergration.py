...
EXTRACTOR_DATA = [
    pytest.param(
        "radiomics", "renal_vision.features.embeddings_radiomics", "RadiomicsExtractor", 107
    ),
...
class TestFeatureExtraction:
    ...
    def test_end_to_end_extraction_real(self, name, mod_path, cls_name, expected_emb_len):
        # 1. Setup
        image = (
            "/sc-scratch/sc-scratch-cc06-ag-ki-radiologie/kits23/dataset/case_00001/imaging.nii.gz"
        )
        seg = "/sc-scratch/sc-scratch-cc06-ag-ki-radiologie/kits23/dataset/case_00001/segmentation.nii.gz"
        extractor = get_extractor(mod_path, cls_name, min_volume=400)

        # 2. Execution
        embeddings = extractor.extract(image, seg)

        # 3. Validation
        assert len(embeddings) == 4, f"{name} should return exactly four embeddings"
